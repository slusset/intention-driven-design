...
10. `specs/modules.yaml` (assigns every capability chain to exactly one module and declares the acyclic module dependency graph)
...
5. Assign the capability to one module; declare module dependencies explicitly rather than inferring them from file placement.
6. If work is delegated or parallelized, define role contracts before edits begin.
